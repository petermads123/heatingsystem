...
halted on nothing and section 3 records only placement judgments, so the orchestrator's own
angle added nothing. One stale README line the integrator lens found (the `fixed_output`
comment still said only `ValueError`) was fixed under `/small-change` before this list was
written. Rounds 3 (state snapshot) and 4 (modulator split) are already decided and are not
re-proposed; several items below are best folded into them and say so.
